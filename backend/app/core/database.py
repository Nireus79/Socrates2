...
def _create_engine(url: str):
    ...
    if url and not url.startswith("sqlite"):
        engine_kwargs.update({
            ...
            "pool_size": 5,  # REVERTED: Normal size (agents now release connections before API calls)
            "max_overflow": 10,  # REVERTED: Normal overflow (root cause of exhaustion fixed)

...
from ..base import BaseDomain, Question, ExportFormat, ConflictRule, QualityAnalyzer
...
class DevOpsDomain(BaseDomain):
    """DevOps and infrastructure operations domain."""

    domain_id = "devops"
    name = "DevOps & Infrastructure"
    version = "1.0.0"
    description = (
        "Infrastructure as code, deployment pipelines, monitoring, "
        "disaster recovery, and operational excellence"
    )

    def __init__(self):
        """Initialize DevOps domain and load all configuration from files."""
        super().__init__()
    ...
    def _load_questions(self) -> None:
        ...
        try:
            ...
            logger.info(f"Loaded {len(self._questions)} devops questions")

        except Exception as e:
            logger.error(f"Failed to load devops questions: {e}")
            self._questions = []
    ...
    def _load_exporters(self) -> None:
        ...
        try:
            ...
            logger.info(f"Loaded {len(self._exporters)} devops exporters")

        except Exception as e:
            logger.error(f"Failed to load devops exporters: {e}")
            self._exporters = []
    ...
    def _load_rules(self) -> None:
        ...
        try:
            ...
            logger.info(f"Loaded {len(self._rules)} devops rules")

        except Exception as e:
            logger.error(f"Failed to load devops rules: {e}")
            self._rules = []
    ...
    def _load_analyzers(self) -> None:
        ...
        try:
            ...
            logger.info(f"Loaded {len(self._analyzers)} devops analyzers")

        except Exception as e:
            logger.error(f"Failed to load devops analyzers: {e}")
            self._analyzers = []

...
# Import Intent Parser for natural language command parsing
...
class SocratesCLI:
    ...
    def __init__(self, api_url: str, debug: bool = False, auto_start_server: bool = True):
        ...
        self.intent_parser = IntentParser(api=self.api, console=self.console)
...
"""
    ...
    def handle_chat_message(self, message: str):
        ...
        if intent:
            # User said something that looks like a command
            confidence = intent.get('confidence', 0)
            requires_confirmation = intent.get('requires_confirmation', False)

            # For low confidence or actions needing confirmation, ask user
            if requires_confirmation or confidence < 0.9:
                command = intent.get('command', '')
                args_str = ' '.join(intent.get('args', []))
                full_cmd = f"{command} {args_str}".strip()

                self.console.print(f"\n[Intent Parse] {intent.get('intent', 'unknown')}")
                self.console.print(f"Confidence: {confidence:.0%}")
                self.console.print(f"Command: {full_cmd}\n")

                if Confirm.ask("Execute this command?"):
                    self.handle_command(full_cmd)
                    return
                else:
                    self.console.print("Treating as chat message instead.\n")
            else:
